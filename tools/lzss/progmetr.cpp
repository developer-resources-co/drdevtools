...
progressMeter::progressMeter( unsigned long lower, unsigned long upper )
	...
	struct text_info ti;
	gettextinfo( &ti );
	Create( (int)ti.curx, (int)ti.cury, lower, upper );
#endif

#ifdef __WATCOMC__
	struct rccoord old_pos;
	old_pos = _gettextposition();
	Create( (int)old_pos.col, (int)old_pos.row, lower, upper );
#endif
	}
...
progressMeter::proc( unsigned long lStep )
	...
	int new_percentage = lStep ? (lStep*100L)/(upper_pm-lower_pm) : 0;
...
char
	progressMeter::displayChar( unsigned long )
	{
	return( 'Û' );
	}
