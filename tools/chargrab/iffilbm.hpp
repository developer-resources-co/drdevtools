...
public:
	...
	// + probably a constructor which takes a rastport & initializes a BMHD (?)
	};
...
protected:
	errorcode decompLine( iffRead* readPtr, ubyte* line_ptr );
	errorcode decompPlaneLine( iffRead* readPtr, ubyte *plane_line_ptr, int byte_length, ubyte compression);

public:
	ilbmBMHD* bmhd;
	rastGraphPort* rp;

	ilbmBODY( iffRead* s, ilbmBMHD* bmhd, rastGraphPort* rp );

	// File I/O
	friend iffWrite& operator<<( iffWrite& s, ilbmBODY& body )
		{ body.Write( &s );  return s; }
	friend iffRead& operator>>( iffRead& s, ilbmBODY& body )
