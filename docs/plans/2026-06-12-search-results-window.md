...
- Wired `OPEN SEARCHLIST` (command layer, `command.cpp`) to `OpenSearchWindow` for parity with
  the other hard-windows. **But the `OPEN <window>` command is pre-existingly broken** in SNES
  builds — `OPEN MEMORY` / `OPEN SYMBOL` also return "No such window", so the typed-command
  window-name table is misaligned (name-table index ≠ `RW_*` enum value, independent of this
  change). Logged as its own TODO; the search window opens fine via the MemOps menu meanwhile.
