...
static boolean FindLine(FILE *, char *, char *, char *, FILE *);
static boolean FindApp(FILE *, char *, FILE *);
...
static boolean
	...
	char szMatch[100];
	char szBuff[256];
	boolean bFoundApp = boolean::FALSE;
	...
	while ( ( !feof( fp ) ) && ( fgets( szBuff, 256, fp ) ) )
		{
		if ( fpo )
			{
			fprintf( fpo, szBuff );
...
static boolean FindLine( FILE *fp, char * lpKeyName,
	char * lpApplicationName, char *szLine, FILE *fpo )
	{
	char szMatch[100];
	char szBuff[256];
	char szOriginal[256];

	boolean bFoundKeyname = boolean::FALSE;
 ...
 *
 */
int GetPrivateProfileInt(char* lpApplicationName, char* lpKeyName,
		int nDefault, char* lpFileName)
	{
	FILE *fp;
	char szLine[256];
 ...
 *
 */
int  GetPrivateProfileString(char* lpApplicationName, char* lpKeyName,
	char* lpDefault, char* lpReturnedString, int nSize, char* lpFileName)
	{
	FILE *fp;
	char szLine[256];
	int nReturn;
	...
	if ( ( fp = fopen( lpFileName, "r+b" ) )!=0)
		...
		if ( FindLine( fp, lpKeyName, lpApplicationName, szLine, NULL ) )
			...
			if (( szEquals = strchr( szLine, '=' ) )!=0)
				{
				strncpy( lpReturnedString, szEquals+1, nSize );
				}
			}
		fclose( fp );
		}
 ...
 */
boolean
	WritePrivateProfileString(char *lpApplicationName, char *lpKeyName,
		char *lpString, char *lpFileName)
	{
	FILE *fp;
	FILE *fpo;
...
boolean
	WritePrivateProfileInt(char *lpApplicationName, char *lpKeyName,
	int nValue, char *lpFileName)
	{
	char szBuff[256];
...
int
	GetProfileString(char *lpApplicationName, char *lpKeyName,
		char *lpDefault, char *lpReturnedString, int nSize )
	{
