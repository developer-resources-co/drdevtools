...
MODEL LARGE
	...
	extrn	C slaveBufferCmdc:DWORD
	extrn 	C slaveCtrlc:WORD
	extrn 	C slaveCtrlBits:BYTE
	extrn 	C slaveRunningCtrlBits:BYTE
	extrn 	C slaveBankc:WORD
	extrn 	C slaveWormc:WORD
	extrn 	C slaveDead:WORD
	extrn 	C snesStatus:WORD
...
STATF_BYTEREADY	equ	1 SHL STATB_BYTEREADY

STATB_BYTERECD	equ	1
STATF_BYTERECD	equ	1 SHL STATB_BYTERECD

; (READ ONLY) Slave is busy processing a request
STATB_BUSY	equ	5
STATF_BUSY	equ	1 SHL STATB_BUSY

; (READ ONLY) Message waiting.  Retrieve & ACK.
STATB_ASYNC	equ	6
STATF_ASYNC	equ	1 SHL STATB_ASYNC

; (READ ONLY) 0=game not running, ...???
STATB_READY	equ	7
STATF_READY	equ	1 SHL STATB_READY
...
GetSlaveBus    PROC FAR
	Cprefix
;;	mov	ax,seg slaveWormc
;;	assume	ds:seg slaveWormc
;;	mov	ds,ax
	mov	dx,slaveWormc
...
ReturnSlaveBus    PROC FAR
	Cprefix
;;	mov	ax,seg slaveCtrlc
;;	assume	ds:seg slaveCtrlc
;;	mov	ds,ax
	mov	dx,slaveCtrlc
	mov	al,slaveCtrlBits
;	mov	al,0e0h
...
GetAttention PROC FAR
    Cprefix
	mov     dx,slaveWormc		; first check if already have attention
	in      al,dx
	and     al,STATF_READY
...
SendCmd PROC FAR
	Cprefix
	push	di
;;	assume	ds:seg slaveBufferCmdc
;;	mov	ax,seg slaveBufferCmdc
;;	mov	ds,ax
	call	GetSlaveBus
	mov     ax,[bp]
	les	di,[slaveBufferCmdc]
	stosb
	call	ReturnSlaveBus
	mov	dx,slaveWormc
