...
//	DevelopmentSystemBoard( Port nBase = 0x304 );
	DevelopmentSystemBoard( Port nBase = 0x318, unsigned short slaveMemSeg = 0xD000 );
	...
	ubyte wormhole( ubyte wh )
#ifdef EMUL
		{ return( 0 ); }
#else
		{ outportb( slaveWormhole, wh ); return( wh ); }
#endif
	ubyte wormhole( void )
#ifdef EMUL
		{ return( 0 ); }
#else
			{ return( inportb( slaveWormhole ) ); }
#endif
	...
	////////////////////////////////////////////////////////////////////////////
	// Communications RAM
	word command( void );			// Read a command
	word command( uword cmd );		// Write a command
